...
def on_message_received(channel, method, properties, body):
    ...
    while not encontrado:
        intentos = intentos + 1
        randomNumber = str(random.randint(1, data["numMaxRandom"]))

        hashCalculado = calculateHash(
            randomNumber + data["baseStringChain"] + data["blockchainContent"]
        )
        if hashCalculado.startswith(data["prefijo"]):
            ...
            print(
                f"Result found and posted for block ID {data['blockId']} in {processingTime:.2f} seconds in {intentos} intentos"
            )
            print(f"Resultado: {randomNumber}")
            print("")

        if (
            intentos > data["numMaxRandom"]
        ):  # salvaguarda para no quedar en loop infinito
